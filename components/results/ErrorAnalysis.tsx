...
export function ErrorAnalysis({ session }: ErrorAnalysisProps) {
  ...
  return (
    <div className="w-full flex flex-col gap-4 sm:gap-6 py-4 sm:py-6 border-t border-subtle">
      ...
      <div className="grid grid-cols-1 xs:grid-cols-3 gap-3 sm:gap-4 font-mono">
        <div className="bg-surface/50 border border-subtle rounded p-3.5 sm:p-4">
          <span className="text-xs text-muted block mb-1">Total Errors</span>
          <span className="text-xl sm:text-2xl font-bold text-error">{errors}</span>
        </div>

        <div className="bg-surface/50 border border-subtle rounded p-3.5 sm:p-4">
          <span className="text-xs text-muted block mb-1">Incorrect Chars</span>
        ...
        <div className="bg-surface/50 border border-subtle rounded p-3.5 sm:p-4">
          <span className="text-xs text-muted block mb-1">Backspaces</span>
          <span className="text-xl sm:text-2xl font-bold text-foreground">{backspaces}</span>
        </div>
      </div>

      {/* Common Mistake Pairs */}
      <div className="bg-surface/40 border border-subtle rounded p-3.5 sm:p-4 font-mono">
